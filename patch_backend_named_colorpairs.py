...
ANCHOR_2 = """    const referenceImages = refRes.rows.map(r => ({ base64: r.file_data.toString('base64'), mime: r.mime_type || 'image/png' }));
    return { brandName, logoDataUrl, referenceImages, aiContextText, colorPairs };
  } catch (e) { console.error('getProjectDesignAssets:', e.message); return null; }
}"""
assert content.count(ANCHOR_2) == 1, f"ANCHOR_2 count = {content.count(ANCHOR_2)}"
NEW_2 = """    const referenceImages = refRes.rows.map(r => ({ base64: r.file_data.toString('base64'), mime: r.mime_type || 'image/png' }));
    return { brandName, logoDataUrl, referenceImages, aiContextText, colorPairs };
  } catch (e) { console.error('getProjectDesignAssets:', e.message); return null; }
}
...
print("OK: getProjectDesignAssets wyciaga nazwane kolory (np. Midnight Blue #0A1F5C) + nowy endpoint GET /api/projects/:id/color-pairs")
