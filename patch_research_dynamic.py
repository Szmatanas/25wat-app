...
ANCHOR_FN = "async function getProjectBrandContext(projectId) {"
assert content.count(ANCHOR_FN) == 1, f"ANCHOR_FN count = {content.count(ANCHOR_FN)}"

NEW_FN = """async function getProjectCompetitors(projectId) {
...
content = content.replace(ANCHOR_FN, NEW_FN, 1)

ANCHOR_HANDLER = """app.post('/api/research/auto', async (req, res) => {
  try {
    const results = [];
    const now = new Date();
...
NEW_HANDLER = """app.post('/api/research/auto', async (req, res) => {
...
print("OK: research/auto czyta konkurentow (competitors) i temat trendow (trends_focus) per projekt, fallback na hardkod 25wat")
